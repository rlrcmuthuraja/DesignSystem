...
document.addEventListener('DOMContentLoaded', function(event) {
    ...
    if ( ! Modernizr.objectfit ) {
        ...
        objectFitImages = document.querySelectorAll('.mrx-objectfit .mrx-objectfit__img');
        ...
        for (var i = 0; i < objectFitImages.length; i++) {
            ...
            if (imageList[i]) {
                ...
                objectFitParent = getClosest(objectFitImages[i], '.mrx-objectfit');

                // Add background-image and class
                objectFitParent.style.backgroundImage = 'url("' + imageList[i] + '")';
                objectFitParent.classList.add('mrx-objectfit--compat');
            }
        }
    }
});